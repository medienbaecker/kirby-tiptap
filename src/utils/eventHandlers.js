 ...
 */
export function createDropHandler(editorRef, panel, helper, endpoints, uploads) {
  return (view, event, slice, moved) => {
    if (!moved && panel.drag.data) {
      ...
      if (panel.drag.type === "text") {
        ...
        handleFileUpload(editorRef, coordinates, event, endpoints, uploads, panel)
      }
 ...
 * @param {Object} panel - Kirby panel instance
 */
function handleFileUpload(editorRef, coordinates, event, endpoints, uploads, panel) {
      ...
      on: {
          ...
          try {
            ...
            if (uploadedFiles?.length > 0) {
              const contents = uploadedFiles.map(fileArray => {
              ...
              if (contents.length > 0) {
                const prevChar = pos > 0 ? editorRef.value.state.doc.textBetween(pos - 1, pos) : ''
                const needsSpace = prevChar && prevChar !== ' '
                const finalContent = needsSpace ? ' ' + contents.join('\n\n') : contents.join('\n\n')
            ...
            panel.notification.error(`Failed to process uploaded file: ${error.message}`)
          }
        },
        cancel: () => {
        ...
        error: (error) => {
          ...
          panel.notification.error(`Upload failed: ${error.message || 'Unknown error'}`)
          // Restore selection to original position
          editorRef.value.commands.setTextSelection({ from, to })
        }
      }
